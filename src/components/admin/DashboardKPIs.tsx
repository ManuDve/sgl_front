...
function KPICard({ label, value, accent = "text-sgl-white" }: KPICardProps) {
  return (
    <div className="bg-sgl-gray border border-sgl-gold/10 rounded-lg px-5 py-6 flex flex-col gap-2">
...
export default function DashboardKPIs() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:8080/api/admin/appointments", {
      ...
      .then((body) => {
        if (body) setKpis(computeKPIs(body.data ?? []));
      })
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
      <KPICard label="Pendientes"  value={String(kpis.pendientes)}  accent="text-sgl-gold" />
      <KPICard label="Confirmados" value={String(kpis.confirmados)} accent="text-green-400" />
      <KPICard label="Cancelados"  value={String(kpis.cancelados)}  accent="text-red-400" />
      <KPICard label="Ingresos"    value={formatMonto(kpis.ingresos)} />
    </div>
  );
}
